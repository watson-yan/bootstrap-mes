<template>
  <div>
    <h2 class="text-center mt-5">Css规范</h2>
    <div class="p-5">
      <h4>语法</h4>
      <ul>
      ...
      <br><br>
      <h4>不要使用 @import</h4>
      <p>与 &lt;link&gt; 标签相比，@import 指令要慢很多，不光增加了额外的请求次数，还会导致不可预料的问题。替代办法有以下几种：</p>
      <ul>
        <li>使用多个 &lt;link&gt; 元素</li>
        <li>通过 Sass 或 Less 之类的 CSS 预处理器将多个 CSS 文件编译为一个文件</li>
        <li>通过 Rails、Jekyll 或其他系统中提供过 CSS 文件合并功能</li>
      </ul>
      <pre class="hl-pre" v-html="$htmlCode(htmlTemp1)">
      </pre>
      <br><br>
      ...
      <br><br>
      <h4>简写形式的属性声明</h4>
      <p>在需要显示地设置所有值的情况下，应当尽量限制使用简写形式的属性声明。常被滥用的简写属性如下：</p>
      <ul>
        <li>padding</li>
        <li>margin</li>
        <li>font</li>
      ...
      <p>大部分情况下，我们不需要为简写形式的属性声明指定所有值。例如，HTML 的标题元素只需要设置上、下边距（margin）的值，因此，在必要的时候，只需覆盖这两个值就可以了。`0` 值表示对浏览器默认值或以前指定的值的覆盖。</p>
...
@Component
export default class GuideCss extends Vue {
  @Provide() htmlTemp1 = `
  <!-- Use link elements -->
